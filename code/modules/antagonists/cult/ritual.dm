...
/obj/item/melee/cultblade/dagger/proc/check_rune_turf(turf/T, mob/user)
	...
	if((!is_station_level(T.z) && !is_mining_level(T.z)) || !(A?.area_flags & CULT_PERMITTED))//SPLURT EDIT: Removes forceful cult prevention ruining AGR
		to_chat(user, "<span class='warning'>The veil is stronger here... This does not feel right..</span>")
